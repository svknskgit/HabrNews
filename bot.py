...
def on_start(update, context):
    ...
    context.bot.send_message(chat_id=chat.id, text='Привет, бот ведет поиск статей по ключевым словам на сайте : ' + 'habr.com' + '\n' +
                                                   'Поиск ведется до нахождения любого слова(фразы) из списка в названии,preview-тексте или полном тексте статьи' + '\n\n' +
                                                   'Введите список ключевых слов(фраз без запятых) для поиска (через запятую)')
    ...
    for npage in range(1, 11):
        ...
        if len(ArticleElem) > 0:
            ...
            for area in ArticleElem:
                article_preview = area.find('div', class_='article-formatted-body')
                article_preview_text = str(article_preview)
                print(article_preview_text)
                ...
                # res.raise_for_status()
                    ...
                    continue
                full_text = article_title + article_preview_text + article_text
                #print('for key_word in KEYWORDS:')
